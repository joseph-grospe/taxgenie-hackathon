...
Environment:
  ...
  - Set TAXTRACK_WORKER_IMAGE_FORCE=1 to rebuild even when worker source is unchanged
...
IMAGE_SOURCE_HASH="$(
  taxtrack_compute_image_source_hash "${ROOT_DIR}" "worker" "${WORKER_HASH_PATHS[@]}"
)"
SKIP_IMAGE_BUILD=0

if [[ -z "${TAG_INPUT}" && "${TAXTRACK_WORKER_IMAGE_FORCE:-}" != "1" ]]; then
  CACHED_IMAGE_URI="${TAXTRACK_WORKER_IMAGE_URI:-}"
  CACHED_IMAGE_SOURCE_HASH="${TAXTRACK_WORKER_IMAGE_SOURCE_HASH:-}"
  ...
  if [[ -n "${CACHED_IMAGE_URI}" &&
    ...
    if CACHED_REPOSITORY="$(taxtrack_image_repository_from_uri "${CACHED_IMAGE_URI}")" &&
      ...
      if taxtrack_ecr_image_exists "${CACHED_IMAGE_URI}" "${AWS_REGION}"; then
        ...
        SKIP_IMAGE_BUILD=1
        echo "Worker image cache hit: ${IMAGE_SOURCE_HASH}"
      else
        echo "Worker image cache entry missing in ECR; rebuilding."
      fi
    else
      echo "Worker image cache repository differs from ${ECR_REPOSITORY}; rebuilding."
    fi
  fi
fi
...
if [[ "${SKIP_IMAGE_BUILD}" != "1" ]]; then
  ...
  persist_env_value "${ENV_FILE}" "TAXTRACK_WORKER_IMAGE_SOURCE_HASH" "${IMAGE_SOURCE_HASH}"
  echo "Updated ${ENV_FILE} with TAXTRACK_WORKER_IMAGE_URI='${IMAGE_URI}'"
  echo "Updated ${ENV_FILE} with TAXTRACK_WORKER_IMAGE_SOURCE_HASH='${IMAGE_SOURCE_HASH}'"
else
  echo "Reusing worker image='${IMAGE_URI}'"
fi
