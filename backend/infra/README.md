...
| `TAXTRACK_ELECTRICSQL_INSTANCE_TYPE` | `t3.small` | `t3.small` | `t3.small` | ElectricSQL EC2 size. |
...
The stack outputs include `infraSizingProfile`, `workerInstanceType`, `dbInstance`, `dbStorageGb`, `dbBackupRetentionDays`, `langfuseInstanceType`, and Batch sizing values. For UAT, verify `infraSizingProfile=uat`, worker `m7i.large`, RDS `t4g.medium` with `100` GB storage, `dbBackupRetentionDays=7`, Langfuse `t3.small`, and Batch `4 vCPU / 16 GB / 80 GiB` with `maxVcpus=16`. For prod, verify `infraSizingProfile=prod` and `dbBackupRetentionDays=30`.
...
- The tunnel requires AWS CLI v2, the Session Manager plugin, IAM permission to start SSM sessions, and a deployed `workerInstanceId`/`dbHost` pair from the SST outputs. Stages without worker EC2 need another SSM-enabled instance in the same VPC/security path.
