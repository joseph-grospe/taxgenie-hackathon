...
const drizzleMigrationsRelativePath = "webapp/tax-genie/src/lib/migrations";
...
function parseLocalDatabaseUrl(databaseUrl: string): LocalDatabaseConfig {
  ...
  const database = parsed.pathname.replace(/^\//, "") || "taxgenie";

  return {
    host: parsed.hostname || "localhost",
    port: Number(parsed.port || "5432"),
    database,
    username: decodeURIComponent(parsed.username || "taxgenie"),
    password: decodeURIComponent(parsed.password || "taxgenie"),
  };
}
...
function validateRdsPasswordFromEnv(): void {
  const password = process.env.TAXGENIE_DB_PASSWORD;
  if (!password) {
    return;
  }
  ...
  if (invalidReason) {
    throw new Error(`TAXGENIE_DB_PASSWORD is invalid for RDS: ${invalidReason}.`);
  }
}
...
export function createData(
  ...
  const dbPassword = requiredSecret("dbPassword", "TAXGENIE_DB_PASSWORD");
