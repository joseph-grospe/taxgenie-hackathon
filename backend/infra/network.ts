...
export function createNetwork(
  ...
  const primaryAz = process.env.TAXGENIE_AZ_PRIMARY ?? `${ctx.region}a`;
  const secondaryAz = process.env.TAXGENIE_AZ_SECONDARY ?? `${ctx.region}b`;
...
cat >/etc/sysctl.d/99-taxgenie-nat.conf <<'CONF'
