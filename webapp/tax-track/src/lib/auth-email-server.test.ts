...
describe('auth-email-server', () => {
  ...
  it('builds verification email bodies without including a temporary password', () => {
    const input = {
      ...
      url: 'https://taxtrack.example.com/api/auth/verify-email?token=abc',
      token: 'abc',
    }
    ...
    expect(html).toContain('<a href="https://taxtrack.example.com')
  ...
  it('sends SES text and HTML email to the resolved recipients', async () => {
    ...
    expect(getSendCommandInput()).toMatchObject({
      Source: 'TaxTrack <verify@taxtrack.online>',
      Destination: {
      ...
      Message: {
        Subject: {
          Charset: 'UTF-8',
          Data: 'Verify your TaxTrack email',
        },
        Body: {
          Text: {
            Charset: 'UTF-8',
