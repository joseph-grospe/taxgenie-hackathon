...
- `webapp/tax-genie/` is the TanStack Start frontend (routes in `src/routes`, UI in `src/components`).
...
- Frontend: routes are file-based in `webapp/tax-genie/src/routes` (kebab-case files, e.g., `batch-status.tsx`).
- Components live in `webapp/tax-genie/src/components`; shared UI in `src/components/ui`.
